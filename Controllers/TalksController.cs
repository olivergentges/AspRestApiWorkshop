...
using AspRestApiWorkshop.Models.Profiles;
using AutoMapper;
    ...
    public class TalksController : ControllerBase
    {
        ...
        public async Task<ActionResult<TalkModel[]>> GetTalks(string moniker)
        {
            try
            {
                ...
                return talkModels.Select(talk => this.CreateLinksForTalk(moniker, talk)).ToArray();
        ...
        public async Task<ActionResult<TalkModel>> GetTalk(string moniker, int id)
        {
            try
            {
                ...
                return this.CreateLinksForTalk(moniker, _mapper.Map<TalkModel>(talk));
            }
            catch (Exception)
            {
                return StatusCode(StatusCodes.Status500InternalServerError, "Database failure");
        ...
        private TalkModel CreateLinksForTalk(string moniker, TalkModel talkModel)
        {
            talkModel.Links.Add(new LinkDto(
                _linkGenerator.GetUriByAction(HttpContext, "GetTalk", "talks", new { version = "2.0", moniker, id = talkModel.TalkId }),
                "self",
                "GET"));
            ...
            talkModel.Links.Add(new LinkDto(
                _linkGenerator.GetUriByAction(HttpContext, "Post", "talks", new { version = "2.0" }),
                "post_talk",
                "POST"));
